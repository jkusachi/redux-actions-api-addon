  ...
  describe("Testing Custom Endpoints", () => {
    ...
    it("test POST with Custom Endpoint", () => {
      const customEndpoint = params =>
        `/user/${params.id}/ronald/${params.name}`;
    ...
    it("test Custom Payload Creator with Custom Meta", () => {
      const customPayloadCreator = ({ city, state }) => ({
        city
      });

      const customMetaCreator = ({ city, state }) => ({
        state
      });
      ...
      const payload = actionCreator({ city: "irvine", state: "ca" });
